...
def decide_market_orders(ctx, state):
    ...
    # 2) LIVESTOCK — RESERVE SPACE, BUY ANIMALS IN STAGES.
    #
    # The tile planner reserves 2 COOP + 6 PASTURE spaces from the beginning.
    # We intentionally DO NOT purchase all 8 animals immediately because their
    # production has a delayed payoff. Early capital must first establish crops,
    # seeds, workers and the first harvest cycle.
    target = counts["target_animals"]
    current = counts["animals"]

    if ctx.day >= 5:
        # Staged livestock ramp:
        # day 5  -> 2 geese
        # day 10 -> +1 cow +1 sheep
        # day 14 -> +1 cow +1 sheep
        # day 18 -> +1 cow +1 sheep
        staged = {"GOOSE": 2, "COW": 0, "SHEEP": 0}
        ...
        # Never exceed the reserved animal slots.
        for animal in staged:
            staged[animal] = min(staged[animal], target.get(animal, 0))

        # At most one new animal per market decision. This prevents livestock
        # from draining a newly harvested cash spike in one turn.
        for animal in ("GOOSE", "COW", "SHEEP"):
            need = staged[animal] - current.get(animal, 0)
            if need <= 0 or slots <= 0:
                continue

            cost = ANIMALS[animal]["cost"]

            # Keep a working runway after buying the animal.
            runway = 700 if ctx.day < 10 else 500 if ctx.day < 14 else 350

            if money >= cost + runway:
    ...
    # 3) SEEDS — BUY A SMALL WAVE, NOT THE WHOLE 92-CROP PLAN.
    #
...
if __name__ == "__main__":
    ...
    with open("v7_vs_starter_replay.json", "w", encoding="utf-8") as f:
        json.dump(env.toJSON(), f)

    print("Replay saved: v7_vs_starter_replay.json")
