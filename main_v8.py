...
class GameState:
    ...
    def reset(self):
        ...
        self.daily_productive = 0
        self.daily_moving = 0
        self.past_capacities = []

    def get_empirical_capacity(self, default=6.0):
        if not self.past_capacities:
            return default
        recent = self.past_capacities[-5:]
...
def desired_allocation(ctx, state):
    ...
    # 1. Estimate productive capacity based on empirical telemetry
    unlocked_tiles = len(ctx.unlocked)
    productive_capacity = state.get_empirical_capacity(default=6.0)
    ...
    # Never shrink the capacity scale below what we already have planted, to prevent mass abandonment
    current_planned_weight = sum(
        (animal_weight if p.get("structure") in ("COOP", "PASTURE") else crop_weight)
        for p in state.tile_plan.values()
    )
    current_scale = current_planned_weight / max(1.0, raw_weight)
    capacity_scale = max(capacity_scale, current_scale)
...
def decide_hiring(ctx, state):
    ...
    # 2. Determine needed units based on telemetry
    productive_capacity = state.get_empirical_capacity(default=6.0)
    needed_hands = int(math.ceil(planned_workload / max(1.0, productive_capacity))) - 1
...
def _move_to_task(ctx, state, idx, pos, inv):
    ...
    has_wheat = int(_get(inv, "WHEAT", 0)) > 0
    has_fert = int(_get(inv, "FERTILIZER", 0)) > 0
    has_animal = any(int(_get(inv, a, 0)) > 0 for a in ANIMALS)
    ...
    def add(kind, coords, value_fn, urgency_fn=None, carrying=False):
        ...
        if has_animal and kind not in ("place", "shed_drop"): return
        if has_fert and kind not in ("fertilize", "shed_drop"): return
        if has_wheat and kind not in ("animal", "shed_drop"): return
    ...
    def animal_urgency(c):
        ...
        if not isinstance(t, dict):
            return 0
        urg = 0
        if not t.get("fed_today", False) and int(_get(inv, "WHEAT", 0)) > 0:
            dry = int(_get(t, "consecutive_unfed", 0))
            f_urg = 4500 if dry >= 1 else 1800
            if ctx.hour >= 18:
                f_urg += 5000
            elif ctx.hour >= 12:
                f_urg += 1000
    ...
    add("fertilize", ctx.fertilize_tiles,
        lambda c: 80, lambda c: 0, int(_get(inv, "FERTILIZER", 0)) > 0)

    add("dig", ctx.weed_tiles,
        lambda c: 20, lambda c: 0, True)

    # 8. Logistics / Supply Runs
    shed_tiles = [(4,4), (5,4), (4,5), (5,5)]
    if ctx.unfed_tiles and int(ctx.shed.get("WHEAT", 0)) > 0:
        add("supply_wheat", shed_tiles,
            lambda c: 50, lambda c: 4500 if ctx.hour < 12 else 6000, 
            int(_get(inv, "WHEAT", 0)) == 0 and not has_animal and not has_fert)

    if ctx.empty_structs:
        for (x, y, kind, _) in ctx.empty_structs:
            for aname, ainfo in ANIMALS.items():
                if ainfo["structure"] == kind and int(ctx.shed.get(aname, 0)) > 0:
                    add("supply_animal", shed_tiles, lambda c: 500, lambda c: 500, 
                        int(_get(inv, aname, 0)) == 0 and not has_wheat and not has_fert)
                    
    if ctx.fertilize_tiles and int(ctx.shed.get("FERTILIZER", 0)) > 0:
        add("supply_fert", shed_tiles, lambda c: 80, lambda c: 0, 
            int(_get(inv, "FERTILIZER", 0)) == 0 and not has_wheat and not has_animal)
    ...
    for act in [farmer_action] + hand_actions:
        if act:
            if act[0] in ("NORTH", "SOUTH", "EAST", "WEST"):
                state.daily_moving += 1
            elif act[0] != "PASS":
                state.daily_productive += 1

    if ctx.hour == 23:
        if (state.daily_productive + state.daily_moving) > 0:
            frac = state.daily_productive / (state.daily_productive + state.daily_moving)
            state.past_capacities.append(24.0 * frac)
        state.daily_productive = 0
        state.daily_moving = 0
