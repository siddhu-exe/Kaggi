...
# Fertilizer is scarce early and premium crops have the best marginal return.
# Tomato/carrot remain viable without it and are not allowed to consume the
# premium fertilizer reserve.
...
PARAMS = {
    ...
    "feed_days": 3,
    "endgame_day": 26,
    # Expansion is conditional in decide_market_orders: later quadrants are
    # bought only when the existing farm is healthy and projected production
    # can repay land, feed, seed, and labor costs before season end.
    "land_days": [5, 12, 17],
    "land_last_day": 19,
    "max_quadrants": 4,
    "land_reserves": [700, 15000, 25000],
...
def desired_allocation(ctx, state):
    ...
    # Adapt future planting to the shared market and the public opponent farm.
    # This only changes empty planned tiles; existing plants are never uprooted.
    opp = _opp_crop_counts(ctx)
    prices = ctx.prices
    for i, (crop, count) in enumerate(raw):
        if crop not in CROPS:
            continue
        base = CROPS[crop]["base"]
        ratio = float(_get(prices, crop, base)) / max(1, base)
        flood = int(opp.get(crop, 0))
        if crop in PREMIUM_GLUT and (ratio < 0.55 or flood >= 14):
            raw[i] = (crop, max(2, count - 6))
        elif crop in ("CARROT", "TOMATO", "WHEAT") and ratio >= 1.15:
            raw[i] = (crop, count + 4)

    # Release melon cohorts instead of filling every unlocked tile at once.
    # This protects the shared premium price and leaves labor capacity for
    # watering, feeding, and courier work during the expensive opening days.
    if ctx.day < 4:
        raw = [(k, min(n, 8) if k == "MELON" else n) for k, n in raw]
    elif ctx.day < 8:
        raw = [(k, min(n, 16) if k == "MELON" else n) for k, n in raw]
    ...
    # Town demand is a reliable scarcity signal.  Shops consume products every
    # four turns, so bias replacement tiles toward currently demanded goods.
    demanded = set()
    for shop in ctx.unlocked_shops:
        demanded.update(TOWN_DEMAND.get(shop, set()))
    for i, (k, n) in enumerate(plan):
        if k in demanded and k in CROPS:
            plan[i] = (k, n + 2)
...
def decide_hiring(ctx, state):
    ...
    # Scale labor only after land expansion is financially justified. The
    # compact two-quadrant mode stays at eight hands; larger farms need ten or
    # twelve to avoid the all-quadrant watering collapse seen in the replay.
    if len(ctx.unlocked_quadrants) >= 4 and ctx.money >= 25000:
        target_hands = 12
    elif len(ctx.unlocked_quadrants) >= 3 and ctx.money >= 15000:
        target_hands = 10
    else:
        target_hands = HANDS_PER_DAY
    total_hands = target_hands + dedicated_hands
...
def decide_market_orders(ctx, state):
    ...
    if q < int(PARAMS.get("max_quadrants", 4)) and slots > 0:
        cost = land_costs[q - 1]
        ...
        stable = productive_now / max(1, len(ctx.unlocked)) >= (0.80 if q >= 2 else 0.50) and not animal_failures
        # Later quadrants need a meaningful cash runway.  A farm that cannot
        # staff and seed the new land should keep harvesting its current land.
        projected_runway = max(0, money - cost - reserve)
        # NE is the proven core expansion. SW/SE are high-growth options only
        # after the bank and workforce can absorb their full operating cost.
        required_runway = 300 if q == 1 else (15000 if q == 2 else 25000)
        enough_runway = projected_runway >= required_runway and len(ctx.hands) >= (8 if q == 1 else 10)
        if (ctx.day >= min_day and stable and enough_runway
                and ctx.day <= PARAMS["land_last_day"]):
    ...
    # 7) Selling: Price-reactive, prioritize WOOL/EGG/MELON
...
def decide_sells(ctx, state, feed_reserve, endgame=False):
    ...
    for good in goods:
        ...
        if good in demand_goods and price >= int(BASE_PRICES.get(good, 1) * 0.8):
            n = max(n, max(1, have // 2))
