...
def decide_market_orders(ctx, state):
    ...
    # V5: hiring is a first-class production resource.  Reserve the first
    # HANDS_PER_DAY market slots for HIRE at the start of every day.
    hire_orders = decide_hiring(ctx, state)
    orders = list(hire_orders)
    ...
    # Never let seed/land/sell orders evict today's HIRE orders.
    # HIRE orders are already at the front of the queue.
    if hire_orders:
        remaining_slots = max(0, MAX_MARKET_ORDERS - len(hire_orders))
        return hire_orders + orders[len(hire_orders):len(hire_orders) + remaining_slots]
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("KAGGRICULTURE V5 — DAILY HANDS DIAGNOSTIC")
    print("=" * 70)
    ...
    for opp in ["starter", "random"]:
        ...
        for step_no, step in enumerate(env.steps):
            if step_no % 24 != 0 or not isinstance(step, list):
                continue
            try:
                p = step[0]
                ...
                print(f"DAY {step_no // 24:02d} | diagnostic error: {e}")
        ...
        for i, player in enumerate(final):
            if isinstance(player, dict):
                print(
                    f"PLAYER {i}: reward={player.get('reward')} "
                    f"status={player.get('status')}"
                )

        try:
            with open(f"v5_{opp}_replay.json", "w", encoding="utf-8") as f:
                json.dump(env.toJSON(), f)
            print(f"Replay saved: v5_{opp}_replay.json")
        except Exception as e:
            print("Replay save failed:", e)
